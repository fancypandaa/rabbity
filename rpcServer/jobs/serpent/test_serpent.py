...
class Serpent:
    ...
    def normal_serpent_encrypt(self,chunk_messages,user_key):
        cipher_message=""
        for plain_text in chunk_messages:
            plain_text = help_functions.replace_space_with_zero(plain_text)
            plain_text=help_functions.convert_string_to_binary(plain_text)
            cipher_message += normal_serpent.encrypt(plain_text, help_functions.convertToBitstring(user_key, 256))
        return cipher_message

    def normal_serpent_decrypt(self,cipher_messages,user_key):
        plain_text=""
    ...
    def main(self,opts,args):
        ...
        if mode == "-e":
            ...
            # print("The Cipher text is: ",self.normal_serpent_encrypt(chunk_messages,user_key) )
            cipher_text= self.normal_serpent_encrypt(chunk_messages,user_key)
            data.append(('-c',cipher_text))
            # print("The key is: ", user_key)
            data.append(('-k',user_key))
        elif mode == "-d":
            user_key = options["-k"]
            if not user_key:
            ...
            plain_text_bits = self.normal_serpent_decrypt(bits,user_key)
