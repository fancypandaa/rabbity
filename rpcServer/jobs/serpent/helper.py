...
def replace_space_with_zero(plain_text):
    return plain_text.replace(" ","0");

def replace_zeros_with_spaces(plain_text):
    return plain_text.replace("0"," ").rstrip()

def convert_string_to_binary(plain_text):
    res = ''.join(format(ord(i), '08b') for i in plain_text)
    return res

def convert_binary_to_string(plain_text):
    binary_int = int(plain_text,2)
    byte_num = binary_int.bit_length() +  7 // 8
    bin_array = binary_int.to_bytes(byte_num,"big")
    ascii_text = bin_array.decode()
    return ascii_text

def chunk_and_plain_text_128bit(bin_plain_text):
    message_chunk=[]
    for i in range(0,len(bin_plain_text),16):
        message = bin_plain_text[i:i+16] 
        if len(message) <16:
            x = 16 - len(message)
            message += (' '*x)
...
def chunk_cipher_into_128bit(cipher_message):
    bits=[]
    for i in range(0,len(cipher_message),128):
        sub_cipher = cipher_message[i:i+128]
        if len(sub_cipher) < 128:
            # impossible for current sitation
            print("Fuck Your Self, Hacker !!!! ")
